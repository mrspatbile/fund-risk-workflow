...
```bash 
python -m src.generate_daily_export
```
...
```bash 
python -m src.generate_positions
```

5.2.  PE fund tables only (requires DB schema already exists)
```bash 
python -c "from src.database import get_engine; from src.generate_pe_fund import generate_pe_fund; generate_pe_fund(get_engine())"
```

5.3.  Infra fund tables only (requires DB schema already exists)
```bash 
python -c "from src.database import get_engine; from src.generate_infra_fund import generate_infra_fund; generate_infra_fund(get_engine())"
```
...
## Annex IV export (AIFMD Article 110)
...
## Board Risk Report PDF (AIFMD Article 15)
